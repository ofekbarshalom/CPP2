...
TEST_CASE("SquareMat test") {
    ...
    SUBCASE("throws on mismatched sizes") {
        SquareMat smaller(1);
        REQUIRE_THROWS_AS(a + smaller, invalid_argument);
    }
    ...
    // Modulo tests
    SUBCASE("Modulo throws on scalar is 0") {
        REQUIRE_THROWS_AS(a % 0, invalid_argument);
    }
    ...
    // Division by scalar tests
    SUBCASE("Division throws on scalar is 0") {
        REQUIRE_THROWS_AS(a / 0, invalid_argument);
    }
    ...
    // Power tests
    SUBCASE("Power throws on negative power") {
        REQUIRE_THROWS_AS(a ^ -1, invalid_argument);
    }
    ...
    SUBCASE("Element access out of range") {
        SquareMat x(2);
        REQUIRE_THROWS_AS(x[2][0], out_of_range);
    }

    SUBCASE("Element access with negative index") {
        SquareMat x(2);
        REQUIRE_THROWS_AS(x[-1][0], out_of_range);
    }
